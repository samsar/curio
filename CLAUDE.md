...
- Cluster labels: deterministic term-frequency by default; optional LLM labels when `insight.labeling = "llm"` AND a generation model is reachable (the daemon pings once at startup and silently falls back to term labels if not).
