...
- `internal/store/` — interfaces (`store.go`) + sqlite impls (`sqlite/`). The interface boundary is real and depguard enforces it (`.golangci.yml`): the only non-test importer of `internal/store/sqlite` besides `cmd/curio-daemon` is the test-support package `internal/store/sqlite/sqlitetest`, whose `NewDB` gives tests a migrated throwaway database. Workers take `store.JobQueue`; the API takes `store.JobStore`, which adds listing, counts, metrics and retention.
