...
| Embedding task prefixes | `internal/indexer`, `internal/search` | nomic-embed-text prefixes (`search_document:` at index, `search_query:` at query) applied to embed input only (stored text stays raw). De-anisotropizes the space; changing them requires `curio reindex` |
