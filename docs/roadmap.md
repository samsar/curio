...
> driven by an async `cluster` job. Cluster labels are LLM-generated by
> default (behind a `generator.Generator` interface, with the model auto-pulled
> on startup), falling back to term-frequency labels when no model is available.
